...
$(OUTPUT_DIR)/glooctl-win-amd64: $(SOURCES)
...
release: release-binaries
	hack/create-release.sh github_api_token=$(GITHUB_TOKEN) owner=solo-io repo=$(REPO) tag=v$(VERSION)
	@$(foreach BINARY,$(RELEASE_BINARIES),hack/upload-github-release-asset.sh github_api_token=$(GITHUB_TOKEN) owner=solo-io repo=$(REPO) tag=v$(VERSION) filename=$(BINARY);)
